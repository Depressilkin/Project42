...
#''', (*i,))
    cursor.execute('''
SELECT name, salary FROM Doctors WHERE salary > 50000 AND salary <= 70000
''')
...
''')
    black_list = cursor.fetchall()
    for dprt in black_list:
        print(dprt)

    cursor.execute('''
SELECT number FROM Wards WHERE (building = 5 OR building = 4) AND floor = 2
''')
    black_list = cursor.fetchall()
    for dprt in black_list:
        print(dprt)